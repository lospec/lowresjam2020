...
namespace HeroesGuild.ui.pause_menu
{
    public class PauseMenu : CanvasLayer
    {
        ...
        private static readonly Dictionary<Menu, Color> MenuCoinDecorColor =
            new Dictionary<Menu, Color>
            {
        ...
        public override void _Ready()
        {
            ...
            _pauseMenuMargin =
                pauseMenuControl.GetNode<MarginContainer>("PauseMenuMargin");
            ...
            _equippedItemsMargin =
                _secondRowHBox.GetNode<MarginContainer>("EquippedMargin");
            _equippedItemsHBox =
                _equippedItemsMargin.GetNode<HBoxContainer>("Equipped");
            _equippedWeapon =
                _equippedItemsHBox.GetNode<InventoryItem>("EquippedWeapon");
            ...
            _itemStatsPopup.RectPosition =
                new Vector2(_itemStatsPopup.RectPosition.x, -19);
        ...
        {
            foreach (Node child in _inventoryItemsGrid.GetChildren()) child.QueueFree();
            ...
            foreach (var item in inventory)
            {
                ...
                var itemTextureButton =
                    inventoryItem.GetNode<TextureButton>("MarginContainer/Item");
                itemTextureButton.TextureNormal = inventoryItem.InventoryItemResource;
                inventoryItem.Connect("mouse_entered", _itemStatsPopup,
                    nameof(_itemStatsPopup.OnItem_MouseEntered),
                    new Array {item});
                inventoryItem.Connect("mouse_exited", _itemStatsPopup,
                    nameof(_itemStatsPopup.OnItem_MouseExited));
                itemTextureButton.Connect("pressed", _itemInfoMenu,
                    nameof(_itemInfoMenu.OnItem_Pressed), new
        ...
        private void UpdateEquippedItem()
        {
            ...
            {
                var equippedWeaponTextureButton =
            ...
            {
                var itemButton =
                    equippedItem.GetNode<TextureButton>("MarginContainer/Item");
                equippedItem.Visible =
                ...
                equippedItem.Connect("mouse_entered", _itemStatsPopup,
                    nameof(_itemStatsPopup.OnItem_MouseEntered), new
                        Array {equippedItem.itemName});

                if (equippedItem.IsConnected("mouse_exited", _itemStatsPopup,
                    nameof(_itemStatsPopup.OnItem_MouseExited)))
                    equippedItem.Disconnect("mouse_exited", _itemStatsPopup,
                        nameof(_itemStatsPopup.OnItem_MouseExited));

                equippedItem.Connect("mouse_exited", _itemStatsPopup,
                    nameof(_itemStatsPopup.OnItem_MouseExited));

                if (itemButton.IsConnected("pressed", _itemInfoMenu,
                    nameof(_itemInfoMenu.OnItem_Pressed)))
                    itemButton.Disconnect("pressed", _itemInfoMenu,
                        nameof(_itemInfoMenu.OnItem_Pressed));

                itemButton.Connect("pressed", _itemInfoMenu,
                    nameof(_itemInfoMenu.OnItem_Pressed), new
        ...
        private void OnButton_ButtonDown(BaseButton button)
        {
            ...
            AudioSystem.PlaySFX(AudioSystem.SFXCollection.PauseMenuTabChanged);
            UpdateMenuState();
        }
